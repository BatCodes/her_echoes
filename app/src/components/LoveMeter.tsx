...
export default function LoveMeter() {
  ...
  /* rest pose + idle wobble live in GSAP — CSS transforms would
     override SVG attribute transforms and freeze the needle */
  useEffect(() => {
    const n = needle.current
    if (!n) return
    gsap.set(n, { rotation: -64, svgOrigin: '100 92' })
    if (!reduced) {
      idle.current = gsap.to(n, {
        rotation: -52, duration: 1.7, yoyo: true, repeat: -1,
        ease: 'sine.inOut', svgOrigin: '100 92',
      })
    }
    return () => {
      idle.current?.kill()
