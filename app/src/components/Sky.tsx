...
const STAR_VERT = `
  ...
  void main() {
    ...
    /* capped: the travelling camera must never inflate sprites into a fill-rate bomb */
    gl_PointSize = min(aSize * uPix * (150.0 / -mv.z), 130.0);
...
const DUST_VERT = `
  ...
  void main() {
    ...
    vec4 mv = modelViewMatrix * vec4(pos, 1.0);
    gl_PointSize = min(aSize * uPix * (90.0 / -mv.z), 48.0);
    gl_Position = projectionMatrix * mv;
  }
`
const DUST_FRAG = `
...
const WISH_VERT = `
  ...
  void main() {
    ...
    vec4 mv = modelViewMatrix * vec4(pos, 1.0);
    gl_PointSize = min(4.2 * uPix * (150.0 / -mv.z), 64.0);
    gl_Position = projectionMatrix * mv;
  }
`
const GOLD_FRAG = `
...
const VISIT_VERT = `
  ...
  void main() {
    ...
    gl_PointSize = min(2.6 * uPix * (150.0 / -mv.z), 48.0);
    gl_Position = projectionMatrix * mv;
  }
`
...
const TWO_VERT = `
  ...
  void main() {
    ...
    vA = uOn * tw;
    vec4 mv = modelViewMatrix * vec4(position, 1.0);
    gl_PointSize = min(aSize * uPix * (150.0 / -mv.z), 120.0);
    gl_Position = projectionMatrix * mv;
  }
`
