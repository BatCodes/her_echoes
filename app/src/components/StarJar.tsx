  ...
  useEffect(() => {
    ...
    import('matter-js').then((M) => {
      ...
      const walls = [
        ...
        wall(110, 26, 96, 16),              // mouth ceiling — a released star ghosts through it
      ]
      ...
      for (let i = 0; i < STAR_COUNT; i++) {
        ...
        /* staggered rows near the floor — no overlap at spawn, no ejections */
        const row = Math.floor(i / 5)
        const b = M.Bodies.circle(
          64 + (i % 5) * 23 + (row % 2) * 9,
          226 - row * 24,
          r,
      ...
      const loop = (t: number) => {
        ...
        for (let i = 0; i < bodies.length; i++) {
          ...
          /* any star that somehow slips the glass quietly comes home */
          if (i !== state.freeIdx && (b.position.x < 32 || b.position.x > 188 || b.position.y > 292 || b.position.y < 12)) {
            M.Body.setPosition(b, { x: 75 + Math.random() * 70, y: 140 })
